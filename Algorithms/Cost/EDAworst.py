...
def EDAworst(ft, variables, probabilities, cost):
    """
    ...
    for var in variables:
        ...
        left_tree, left_cost = EDAworst(left_ft, remaining_var, probabilities, cost)
        right_tree, right_cost = EDAworst(right_ft, remaining_var, probabilities, cost)

        expected_cost = cost[var] + (1-probabilities[var])*left_cost + probabilities[var]*right_cost

        if expected_cost > optimal_cost:
            optimal_cost = expected_cost
            optimal_tree = (var, left_tree, right_tree)
    return optimal_tree, optimal_cost


def restrict(ft, var, value):
    """
    Function that evaluates a variable in a fault tree
    :param ft: ft that needs to be restricted
...
if __name__ == "__main__":
    ...
    normalddt, height = EDA(top, B, P)
    print("DDT:", normalddt)
    print("Exp height:", height)
    ddt, expcost = EDAcost(top, B, P, cost)
    print("DDT:", ddt)
