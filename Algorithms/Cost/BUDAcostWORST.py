...
def BUDAcostworst(ft):
    ...
    if ft.type == FtElementType.BE:
        # print(f"name: {ft.name} prob: {ft.prob} cost: {ft.cost}")
        return (ft.name, '0', '1'), ft.prob, ft.cost

    subtrees  = [BUDAcostworst(ch) for ch in ft.children]
    ...
    if ft.type == FtElementType.OR:
        ordered_children = sorted(subtrees, key=lambda x: x[2]/x[1], reverse=True)
